...
for name, code, d_id_var, h_id_var in departments:
    ...
    for i in range(1, 7):
        ...
        sql.append(f"""    INSERT INTO public.users (id, role, email, name, password_hash, is_active, department_id, created_at, updated_at) VALUES (fac_id, 'teacher', '{fac_email}', '{code} Professor {i}', '{fac_pw_hash}', true, {d_id_var}, now(), now());""")

...
def run_test():
    ...
    resp = requests.post(
        f"{BASE_URL}/assignments/create", data=assign_data, headers=teacher_headers
    )
    if resp.status_code == 200:
    ...
    with open("project_submission.txt", "w") as f:
        f.write(
            "This is my AI project submission. It uses LLMs to generate art.\nDocumentation involved."
        )
    ...
    with open("project_submission.txt", "rb") as f:
        ...
        resp = requests.post(
            f"{BASE_URL}/assignments/submit", data=data, files=files, headers=student_headers
        )
    ...
    grade_data = {"score": 95, "feedback": "Excellent work on the capstone! Innovative approach."}
    resp = requests.put(
        f"{BASE_URL}/assignments/{assignment_id}/submissions/{submission_id}/score",
        json=grade_data,
    ...
    resp = requests.get(
        f"{BASE_URL}/assignments/list?student_id={submission['student_id']}&course_id={course_id}",

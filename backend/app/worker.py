...
def task_grade_submission(self, assignment_id: str, submission_id: str, description: str, file_path: str):
    """
    ...
    try:
        ...
        # Grader service uses LLM, might be async, but Celery tasks are typically sync wrappers
        # If grader_service.grade_submission is async, we need async-to-sync bridge
        # Assuming grader_service.grade_submission is accessible. 
        # CAUTION: If grader calls async LLM, we need `asyncio.run`.
        
        # Inspecting grader_service usually shows it might use `google.generativeai` which is sync or async.
        # Let's wrap in asyncio.run just in case it's an async def, or check source.
        # Safest bet: The previous code awaited it. So it IS async.
        
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(grader_service.grade_submission(extracted_text, description))
        loop.close()

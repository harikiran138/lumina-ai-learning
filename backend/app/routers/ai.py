...
async def generate_course(request: CourseGenerationRequest):
    ...
    [NEW RULE] You must also output a concept graph representing dependencies between the key ideas in this course."""
    ...
    {{
        ...
        "modules": [
            {{
                ...
                "assessment_strategy": "Explain the type of quiz or evidence needed for this module"
            }}
        ],
        "concept_graph": {{
            "nodes": [
                {{"id": "node-1", "label": "Concept Name", "level": "core"}}
            ],
            "edges": [
                {{"from": "node-1", "to": "node-2", "relationship": "prerequisite"}}
            ]
        }}
    }}
    ...
    try:
        ...
        saved_course = await store.create_course_from_blueprint(data, teacher_id="system")

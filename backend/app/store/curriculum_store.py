...
class CurriculumStore(BaseStore):
    ...
    async def get_semesters(self, program_id: UUID) -> List[Dict]:
        ...
        result = self.supabase.table("semesters")\
            .select("*")\
    ...
    async def get_student_enrollment(self, student_id: UUID) -> Optional[Dict]:
        ...
        enr_res = self.supabase.table("student_enrollments") \
            .select("*") \
        ...
        sem_res = self.supabase.table("semesters") \
            .select("*") \
        ...
        prog_res = self.supabase.table("programs") \
            .select("*") \
    ...
    async def enroll_student(self, student_id: UUID, program_id: UUID, semester_id: UUID):
        ...
        result = self.supabase.table("student_enrollments")\
    ...
    async def get_allowed_courses(self, student_id: UUID) -> List[Dict]:
        ...
        sem_result = self.supabase.table("semesters")\
            .select("id")\
            .eq("program_id", str(program_id))\
        ...
        courses_result = self.supabase.table("courses")\
            .select("*")\

...
class ParentStore:
    ...
    async def get_child_mastery(self, child_id: str) -> Optional[Dict[str, Any]]:
        try:
            client = self.db.get_client()
            response = client.table("learner_profiles").select("performance_summary, mastery_state").eq("user_id", child_id).execute()
    ...
    async def get_child_assignments(self, child_id: str) -> List[Dict[str, Any]]:
        try:
            ...
            log.error("get_child_assignments_failed", child_id=child_id, error=str(e))
            return []

...
class AssignmentStore:
    ...
    def assignments_collection(self):
        if self._assignments_collection is None:
            ...
            if _db is not None:
                ...
                log.error("mongodb_connection_missing", collection="assignments")
    ...
    def submissions_collection(self):
        if self._submissions_collection is None:
            ...
            if _db is not None:
                ...
                log = structlog.get_logger()
                log.error("mongodb_connection_missing", collection="submissions")
                raise HTTPException(status_code=503, detail="Database connection unavailable")
    ...
    def update_submission_grade(
        ...
        collection.update_one({"id": submission_id}, {"$set": update_data})
    ...
    def get_student_submission(self, assignment_id: str, student_id: str) -> Optional[dict]:
        ...
        if collection is None:
            return None
        doc = collection.find_one({"assignment_id": assignment_id, "student_id": student_id})
        if doc:
            doc.pop("_id", None)
            return doc
        return None

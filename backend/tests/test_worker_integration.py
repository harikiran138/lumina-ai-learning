...
@pytest.mark.skipif(not is_docker_available(), reason="Docker daemon not found")
def test_celery_worker_flow(redis_container, mongo_container):
    ...
    with patch("app.services.storage.storage_service.download_file"), patch(
        "app.services.ocr_service.ocr_service.digitize_image"
    ) as mock_ocr, patch(
        "app.services.grader_service.grader_service.grade_submission"
    ) as mock_grade:
        mock_ocr.return_value = "Mocked Text Content"

        async def async_mock_grade(*args):
            return {"score": 88, "feedback": "Good job"}

        mock_grade.side_effect = async_mock_grade

        # Trigger Task
        result = task_grade_submission.apply(
            args=("assign_1", "sub_1", "Desc", "path/to/file.pdf")
        ).get()

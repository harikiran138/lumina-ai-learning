...
def test_flow():
    ...
    res = requests.post(
        f"{BASE_URL}/auth/token", data={"username": teacher_email, "password": teacher_password}
    )
    if res.status_code != 200:
    ...
    res = requests.post(
        ...
        data={"name": "Test Course 101", "code": "TEST101", "description": "A test course"},
    )
    if res.status_code == 200:
    ...
    res = requests.post(
        f"{BASE_URL}/auth/token", data={"username": student_email, "password": student_password}
    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            "requests library not found. Please install it (pip install requests) to run this test."
        )

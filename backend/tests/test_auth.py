...
def test_auth_security_checks(client):
    ...
    client.post(
        "/api/auth/register",
        json={"email": email_a, "password": "pass", "full_name": "Student A", "role": "student"},
    )
    token_a = client.post("/api/auth/token", data={"username": email_a, "password": "pass"}).json()[
        "access_token"
    ]
    ...
    client.post(
        "/api/auth/register",
        json={"email": email_b, "password": "pass", "full_name": "Student B", "role": "student"},
    )
    token_b = client.post("/api/auth/token", data={"username": email_b, "password": "pass"}).json()[
        "access_token"
    ]

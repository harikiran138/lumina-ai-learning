...
def verify_pathway_logic():
    ...
    try:
        ...
        resp = requests.post(f"{BASE_URL}/api/tutor/chat", json=payload, timeout=60) # Long timeout for LLM
        ...
        if "response" in data and "personalization" in data:
            print(f"✅ Valid Response Structure. Rec: {data['personalization'].get('recommendation')}")

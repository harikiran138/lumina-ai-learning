...
class SimulationRunner:
    ...
    def next_question(self, session_id: str):
        ...
        decision = self.policy.decide_next_action(
            session.mastery_state, history_objs, "arrays"
        )  # Focus on arrays
...
def main():
    ...
    for r in results:
        ...
        print(
            f"{r['id']:<12} | {r['skill']:<10.2f} | {r['q_count']:<10} | {r['acc']:<10.2f} | {str(r['reason']):<30} | {arr_m:.2f}"
        )

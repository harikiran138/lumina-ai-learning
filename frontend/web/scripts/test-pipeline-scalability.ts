...
import {
  chunkPages,
  extractSectionsFromChunk,
  mergeSectionsToCourse,
    ...
    completions: {
      create: async (params: any) => {
                        ...
                        subsections: [
                          {
                            title: "Subsection 1",
                            contentBlocks: [
                              { type: "paragraph", data: "Some content here." },
                            ],
                          },
                        ],
                      },
        ...
        if (prompt.includes("Curriculum Architect")) {
          ...
          console.log(
            `    [Mock Engine] Received prompt with ${ids.length} items to merge.`,
          );
...
async function runTest() {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const result = await extractSectionsFromChunk(
      chunks[i].text,
      mockEngine as any,
    );
  ...
  try {
    ...
    if (course && course.modules && course.modules.length > 0) {
      const totalTopics = course.modules.reduce(
        (acc: number, m: any) => acc + m.topics.length,
        0,
      );
      ...
      console.log(
        `   (Should verify that totalTopics matches input sections if 1-to-1)`,
      );
      ...
      if (totalTopics === allSections.length) {
        ...
        console.log(
          `⚠️ Count Mismatch: Input ${allSections.length} vs Output ${totalTopics}`,
        );
      }

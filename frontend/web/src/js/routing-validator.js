...
class RoutingValidator {
  ...
  testLogoLinks() {
    ...
    const logoLinks =
      document.querySelectorAll('a[class*="gradient-text"]').length > 0
  ...
  validateLogoLink(link) {
    ...
    if (href === expectedHref) {
      ...
      this.addResult(
        "Logo Link",
        "FAIL",
        `Expected: ${expectedHref}, Found: ${href}`,
      );
    }
  }
  ...
  testCrossPageLinks() {
    ...
    loginLinks.forEach((link) => {
      const href = link.getAttribute("href");
      const expectedLoginHref =
        this.currentDir === "root" ? "login.html" : "../login.html";
      ...
      if (href === expectedLoginHref) {
        ...
        this.addResult(
          "Login Link",
          "FAIL",
          `Expected: ${expectedLoginHref}, Found: ${href}`,
        );
      }
    });
  ...
  testAuthenticationRedirects() {
    ...
    if (typeof luminaUI !== "undefined" && luminaUI.redirectToDashboard) {
      ...
      testRoles.forEach((role) => {
        try {
          ...
          this.addResult(
            "Auth Redirect",
            "FAIL",
            `${role} redirect failed: ${error.message}`,
          );
        }
      });
    } else {
      this.addResult(
        "Auth Redirect",
        "FAIL",
        "luminaUI.redirectToDashboard not available",
      );
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    window.location.search.includes("check-routing=true") ||

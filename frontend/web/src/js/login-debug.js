      ...
      try {
        ...
        if (user) {
          ...
          if (user.role !== account.expectedRole) {
            console.warn(
              `⚠️ Role mismatch: expected ${account.expectedRole}, found ${user.role}`,
            );
          }
        } else {
        ...
        console.error(
          `❌ Failed to check account ${account.email}:`,
          error.message,
        );
      }
    }
...
console.log(
  "🔧 Login Debug Tools loaded. Use debugLogin(), testRedirect(role), or testQuickLogin(role) in console.",
);

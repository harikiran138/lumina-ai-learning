...
Student Context: You are currently helping a student in their learning journey. Tailor your responses to be educational and supportive.`;
  ...
  async generateResponse(userMessage) {
    ...
    if (!this.apiKey) {
      throw new Error(
        "Gemini API key not configured. Please set your API key first.",
      );
    }
  ...
  async getSubjectHelp(subject, topic, question = null) {
    const subjectPrompts = {
      math: `Help with ${topic} in mathematics. ${
        question
          ? `Specific question: ${question}`
          : "Provide an overview and examples."
      }`,
      science: `Explain ${topic} in science. ${
        question
          ? `Question: ${question}`
          : "Include key concepts and real-world applications."
      }`,
      history: `Discuss ${topic} in history. ${
        question ? `Question: ${question}` : "Provide context and significance."
      }`,
      literature: `Analyze ${topic} in literature. ${
        question
          ? `Question: ${question}`
          : "Include themes and literary devices."
      }`,
      language: `Help with ${topic} in language learning. ${
        question ? `Question: ${question}` : "Provide examples and practice."
      }`,
    };

    const prompt =
      subjectPrompts[subject.toLowerCase()] ||
      `Help the student understand ${topic}. ${
        question ? `Their question: ${question}` : ""
      }`;
  ...
  async reviewWork(workContent, subject = null) {
    const prompt = `Please review this student work${
      subject ? ` in ${subject}` : ""
    }:
  ...
  isConfigured() {
    return (
      !!this.apiKey &&
      this.apiKey !== "REPLACE_WITH_YOUR_GEMINI_API_KEY_HERE" &&

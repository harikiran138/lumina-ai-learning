...
class CommunityChat {
  ...
  async refreshMessages() {
    if (this.currentRoomId) {
      try {
        this.currentMessages = await this.api.getChatMessages(
          this.currentRoomId,
        );
        this.renderMessages();
      } catch (error) {
  ...
  renderChat(room) {
    ...
    if (members.length > 4) {
      memberAvatars += `<div class="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-300 text-xs font-bold border-2 border-white dark:border-black">+${
        members.length - 4
      }</div>`;
    }
  ...
  renderMessages() {
    ...
    this.currentMessages.forEach((msg) => {
      ...
      const messageGroupClass = isContinuing
        ? "chat-message-group mt-1"
        : "chat-message-group mt-4";
      ...
      if (isCurrentUser) {
        this.messagesContainer.innerHTML += `
                    <div class="flex items-start gap-3 flex-row-reverse ${messageGroupClass}">
                        ...
                        } flex-shrink-0 flex items-center justify-center text-white font-bold chat-avatar">
                            ${this.currentUser.avatar}
                        </div>
                        <div class="flex-1 flex flex-col items-end">
                            ${
                              !isContinuing
                                ? `<p class="font-semibold text-sm">You <span class="text-xs text-gray-500 dark:text-gray-400 font-normal ml-2">${msg.time}</span></p>`
                                : ""
                            }
                            <div class="mt-1 text-sm bg-amber-500 text-white p-3 rounded-lg ${
                              isContinuing ? "" : "rounded-br-none"
                            } max-w-md">
                                ${msg.text}
                            </div>
                        </div>
        ...
        const roleBadge =
          msg.senderRole === "teacher"
            ? `<span class="text-xs bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300 font-bold px-1.5 py-0.5 rounded-full ml-2">TEACHER</span>`
            : msg.senderRole === "admin"
              ? `<span class="text-xs bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300 font-bold px-1.5 py-0.5 rounded-full ml-2">ADMIN</span>`
              : "";
        ...
        this.messagesContainer.innerHTML += `
                    <div class="flex items-start gap-3 ${messageGroupClass}">
                        ...
                        <div>
                            ${
                              !isContinuing
                                ? `<p class="font-semibold text-sm">${msg.sender} ${roleBadge}<span class="text-xs text-gray-500 dark:text-gray-400 font-normal ml-2">${msg.time}</span></p>`
                                : ""
                            }
                            <div class="mt-1 text-sm bg-white dark:bg-[#1C1C1C] p-3 rounded-lg ${
                              isContinuing ? "" : "rounded-tl-none"
                            } border border-gray-200 dark:border-gray-800 max-w-md">
                                ${msg.text}
                            </div>
                        </div>
                    </div>`;

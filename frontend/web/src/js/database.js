...
class LuminaDB {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("assessments")) {
          ...
          assessmentStore.createIndex("courseId", "courseId", {
            unique: false,
          });
        ...
        if (!db.objectStoreNames.contains("progress")) {
          ...
          progressStore.createIndex("studentId", "studentId", {
            unique: false,
          });
        ...
        if (!db.objectStoreNames.contains("systemHealth")) {
          const healthStore = db.createObjectStore("systemHealth", {
            keyPath: "id",
          });
        }
        ...
        if (!db.objectStoreNames.contains("chatRooms")) {
          const chatStore = db.createObjectStore("chatRooms", {
            keyPath: "id",
          });
  ...
  async createCourse(courseData) {
    const course = {
      ...
      contentStatus: courseData.contentStatus || {
        vectorDB: "0%",
        pathways: "0%",
      },
      createdAt: new Date().toISOString(),
  ...
  async seedInitialData(force = false) {
    ...
    if (force || needsSeeding) {
      ...
      const existingDemoAdmin = users.find(
        (u) => u.email === "admin@lumina.com",
      );
      const existingDemoTeacher = users.find(
        (u) => u.email === "teacher@lumina.com",
      );
      const existingDemoStudent = users.find(
        (u) => u.email === "student@lumina.com",
      );
    ...
    const defaultChatRooms = [
      {
        ...
        description:
          "Welcome to Lumina! Chat with students and teachers from all subjects.",
        type: "general",
        members: [
          "admin_001",
          "teacher_001",
      ...
      },
      {
        ...
        name: "Announcements",
        description:
          "Important updates and announcements from teachers and admins.",
        type: "general",
        members: [
          "admin_001",
          "teacher_001",
    ...
    const sampleMessages = [
      {
        roomId: "general",
        senderId: "admin_001",
        text: "Welcome to Lumina Community! 🎉 Feel free to introduce yourselves and connect with fellow learners.",
      },
      {
        roomId: "general",
        senderId: "student_001",
        text: "Hi everyone! Excited to be here. Looking forward to learning together! 📚",
      },
      {
        roomId: "study_buddies",
        senderId: "student_002",
        text: "Anyone want to form a study group for the upcoming physics exam?",
      },
      {
        roomId: "physics_help",
        senderId: "teacher_001",
        text: "Physics help desk is now open! Drop your questions here and I'll help you out. 🔬",
      },
      {
        roomId: "announcements",
        senderId: "admin_001",
        text: "📢 New AI Tutor feature is now available! Check it out in the navigation menu.",
      },
    ];

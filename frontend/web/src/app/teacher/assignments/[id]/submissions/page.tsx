...
import { ArrowLeft, FileText, Download, Calendar, User, Clock, Loader2, Sparkles, X, CheckCircle, BarChart2, BookOpen } from 'lucide-react';
...
export default function AssignmentSubmissionsPage() {
    ...
    const fetchAnalytics = async () => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE || 'http://127.0.0.1:8000'}/api/assignments/${assignmentId}/analytics`, { cache: 'no-store' });
            if (res.ok) {
                const data = await res.json();
                setAnalytics(data);
            }
    ...
    const handleViewReport = async (submissionId: string) => {
        ...
        setCoursePlan(null);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE || 'http://127.0.0.1:8000'}/api/assignments/${assignmentId}/submissions/${submissionId}/report`, { cache: 'no-store' });
            if (res.ok) {
                const data = await res.json();
    ...
    const handleGenerateCoursePlan = async (submissionId: string) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE || 'http://127.0.0.1:8000'}/api/ai/generate-course-from-assignment`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const handleSaveCourseFromPlan = async () => {
        ...
        try {
            ...
            const modules = [
                {
                    ...
                    lessons: (coursePlan.outline || []).map((item: string, idx: number) => ({
            ...
            const result = await api.createCourse({
                ...
                description: coursePlan.description || 'AI-generated personalized learning path',
                modules,
        ...
        <div className="p-6 md:p-8 space-y-8 relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between">
                    <div>
                        <div className="text-xs uppercase tracking-wide text-gray-400 mb-1">Submissions</div>
                        <div className="text-2xl font-bold text-white">{analytics?.submission_count ?? 0}</div>
                    </div>
                ...
                <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between">
                    <div>
                        ...
                        <div className="text-2xl font-bold text-white">
                            {analytics?.avg_grade !== null && analytics?.avg_grade !== undefined
                ...
                <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between">
                    <div>
                        <div className="text-xs uppercase tracking-wide text-gray-400 mb-1">Graded</div>
                        <div className="text-2xl font-bold text-white">{analytics?.graded_count ?? 0}</div>
                        ...
                        <div className="p-6 space-y-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-1 font-mono">
                                        Assignment: {submissionReport.assignment?.title} • Student: {submissionReport.submission?.student_id}
                                    </p>
                                </div>
                                <button onClick={() => { setSubmissionReport(null); setCoursePlan(null); }} className="text-gray-400 hover:text-white">
                                    <X size={22} />
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                                    ...
                                    <div className="text-3xl font-bold text-green-400">{submissionReport.score ?? '--'}<span className="text-lg text-gray-400">/100</span></div>
                                </div>
                                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                                    <div className="text-xs text-gray-400 mb-1">Level</div>
                                    <div className="text-xl font-semibold text-white capitalize">{submissionReport.level ?? 'N/A'}</div>
                                </div>
                                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                                    <div className="text-xs text-gray-400 mb-1">Status</div>
                                    <div className="text-sm text-gray-300">{submissionReport.submission?.status ?? 'unknown'}</div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="bg-black/40 border border-white/10 rounded-xl p-4">
                                    <h3 className="text-sm font-semibold text-gray-200 mb-2">AI Feedback</h3>
                                    <p className="text-sm text-gray-300 whitespace-pre-wrap min-h-[4rem]">
                                        {submissionReport.feedback || 'No feedback available.'}
                                    </p>
                                </div>
                                <div className="bg-black/40 border border-white/10 rounded-xl p-4">
                                    <h3 className="text-sm font-semibold text-gray-200 mb-2">OCR Text (Student Work)</h3>
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-sm font-semibold text-white">AI Personalized Course Plan</h3>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button
                                            onClick={() => handleGenerateCoursePlan(submissionReport.submission?.id)}
                                        ...
                                        >
                                            {courseLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                                        ...
                                        {coursePlan && (
                                            ...
                                            >
                                                {courseSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <SaveIcon />}
                                ...
                                {coursePlan && (
                                    <div className="mt-2 text-left space-y-2">
                                        <div className="font-semibold text-white">{coursePlan.title}</div>
                                        <p className="text-xs text-gray-400 mb-2">{coursePlan.description}</p>
                                        ...
                                        {savedCourseId && (
                                            <p className="text-[10px] text-emerald-400 mt-2">
                                                Saved as draft course. You can manage it under Teacher → Courses.
                                            </p>
                                        )}
                                    </div>
                                )}

  ...
  const handleSendAIQuestion = async () => {
    ...
    try {
      ...
      const context = activeLesson
        ? `Context: You are an AI tutor helping a student with the lesson "${
            activeLesson.title
          }". Content: ${
      ...
      const aiText =
        typeof response === "string"
          ? response
          : response.content || "I couldn't understand that.";
              ...
              {!quizSubmitted && currentQuestion ? (
                    ...
                    {currentQuestion.options.map(
                      (option: string, optIdx: number) => (
                        <button
                          key={optIdx}
                          onClick={() =>
                            handleQuizOptionSelect(currentQuestion.id, optIdx)
                          }
                ...
                <div className="text-center py-12 flex flex-col items-center justify-center h-full animate-in zoom-in-95 duration-500">
                  ...
                  <p className="text-xl text-gray-300 mb-8">
                    You scored{" "}
                    <span className="text-lumina-primary font-bold text-2xl">
                      {quizScore}
                    </span>{" "}
                    / {activeQuiz.questions.length}
                  </p>
                ...
                <div className="flex-1 p-4 overflow-y-auto space-y-4 font-sans text-sm">
                  <div className="bg-white/5 p-3 rounded-xl rounded-tl-none">
                    <p className="text-gray-300">
                      Hi! I'm your AI tutor for this lesson. I have context
                      about "{activeLesson?.title}". What's confusing you?
                    </p>
                  </div>
                  ...
                  {aiChatHistory.map((msg, idx) => (
                    <div
                      ...
                      className={`${
                        msg.role === "user"
                          ? "bg-lumina-primary/10 ml-auto border-lumina-primary/20 rounded-tr-none"
                          : "bg-white/5 rounded-tl-none"
                      } p-3 rounded-xl max-w-[80%] border ${
                        msg.role === "user" ? "border" : "border-transparent"
                      }`}
                    >
                      <p
                        className={
                ...
                <div className="p-4 border-t border-white/10 bg-white/5 rounded-b-2xl">
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleSendAIQuestion()
                      }
                      ...
                      <div className="border-t border-white/10 bg-black/20">
                        {module.lessons?.map((lesson: any, lIdx: number) => (
                          ...
                          >
                            <div
                              onClick={() =>
                                handleLessonSelect(lesson, i, lIdx)
                              }
                              className={`p-4 flex items-center justify-between hover:bg-white/5 cursor-pointer group ${
                                activeLessonIndex?.m === i &&
                                activeLessonIndex?.l === lIdx
                                  ? "bg-lumina-primary/10 border-l-2 border-l-lumina-primary"
                                  : ""
                              }`}
                            >
                                      ...
                                      {(() => {
                                              ...
                                              <div className="space-y-8 animate-in fade-in duration-500">
                                                ...
                                                {parsed.goal && (
                                                  <div className="bg-lumina-primary/10 border-l-4 border-lumina-primary p-4 rounded-r-xl">
                                                    <h4 className="text-lumina-primary font-bold text-sm uppercase tracking-wider mb-1">
                                                      Learning Goal
                                                    </h4>
                                                              ...
                                                              {block.content
                                                                ...
                                                                .map(
                                                                  (
                                                                    ...
                                                                    >
                                                                      <span className="text-lumina-primary mt-1.5">
                                                                        •
                                                                      </span>
                                                                      <span>
                                                                        {item
                                                      ...
                                                      <div className="grid gap-6">
                                                        {parsed.subtopics.map(
                                                          (
                                                            ...
                                                            <div
                                                              key={sIdx}
                                                              className="bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/20 rounded-2xl p-6 transition-all"
                                                            >
                                        ...
                                        return activeLesson.content ? (
                                                ...
                                                (line: string, i: number) => {
                                                  ...
                                                  if (line.startsWith("- "))
                                                    return (
                                                      <li
                                                        key={i}
                                                        className="ml-4 text-gray-300 list-disc"
                                                      >
                                                        {line.replace("- ", "")}
                                                      </li>
                                                    );
                                                  if (line.startsWith("```"))
                                                    return (
                                                      <div
                                                        key={i}
                                                        className="bg-black/50 p-4 rounded-lg my-4 text-sm font-mono text-green-400 border border-white/10 overflow-x-auto"
                                                      >
                                                        Code Block (See render)
                                                      </div>
                                                    );
                                                  if (line.trim() === "")
                                                    return <br key={i} />;
                                          ...
                                          <p className="text-gray-400 italic">
                                            No text content available for this
                                            lesson.
                                          </p>
                                        );
                                      })()}

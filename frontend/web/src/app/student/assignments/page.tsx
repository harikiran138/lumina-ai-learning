...
export default function StudentAssignmentsPage() {
    ...
    const fetchAssignments = async () => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE || 'http://127.0.0.1:8000'}/api/assignments/list?student_id=student_demo`);
            if (!res.ok) {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, assignmentId: string) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE || 'http://127.0.0.1:8000'}/api/assignments/submit`, {
                method: "POST",
                body: formData,
            });

  ...
  const handleHandwritingUpload = async (data: any) => {
    ...
    try {
      if (activeNote) {
        ...
        // Ensure title exists
        const titleToUse =
          newFormData.title || "Digitized Note " + new Date().toLocaleString();
        const createData = { ...newFormData, title: titleToUse };
        if (!newFormData.title)
          setFormData((prev) => ({ ...prev, title: titleToUse })); // Update UI title
        ...
        {viewMode === "editor" ? (
          ...
          <div className="flex flex-col items-center gap-6 pt-2">
            ...
            <button
              ...
              suppressHydrationWarning
              className="p-2 bg-lumina-primary text-black rounded-xl hover:scale-110 transition-transform"
              title="New Note"
            >
      ...
      {viewMode !== "editor" && (
        <div className="lg:hidden px-4 md:px-0 flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
          <button
            onClick={handleCreateNew}
            suppressHydrationWarning
            className="p-2 bg-lumina-primary rounded-full text-black flex-shrink-0"
          >
            <Plus className="w-5 h-5" />
          </button>
          {subjects.map((subject) => (
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
              <div
                className={`flex-1 flex flex-col h-full relative ${
                  previewFile
                    ? "hidden md:flex md:w-1/2 border-r border-white/5"
                    : "w-full"
                }`}
              >
                <textarea
                ...
                <div className="p-4 border-t border-white/5 flex items-center gap-4 bg-black/40 backdrop-blur-sm">
                  ...
                  <div className="flex-1 flex gap-2 overflow-x-auto custom-scrollbar pb-1">
                    {formData.attachments.map((file, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg pl-3 pr-2 py-1.5 text-xs text-gray-300 flex-shrink-0"
                      >
                        <span
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-gray-500 max-w-xs mb-6">
                    Create your first note to start building your personal
                    knowledge base.
                  </p>
                  <button
                    onClick={handleCreateNew}
                    ...
                    {filteredNotes.map((note) => (
                      ...
                      >
                        ...
                        <div className="relative z-10">
                          ...
                          <h3 className="font-bold text-lg text-white mb-2 line-clamp-1 group-hover:text-lumina-primary transition-colors">
                            {note.title}
                          </h3>
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-gray-400 mb-6">
                Upload a photo or PDF of your handwritten notes. We'll extract
                the text using AI and add it to your note automatically.
              </p>
              <HandwritingUpload
                type="note"

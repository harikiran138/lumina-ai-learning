...
export default function ResearcherLayout({ children }: { children: React.ReactNode }) {
  ...
  ? { name: user.name ?? "Researcher", role: "Researcher", initial: (user.name ?? "R").charAt(0), avatar: user.avatar }
  : { name: "Researcher", role: "Researcher", initial: "R" };
          ...
          <div className="relative z-10">
            ...
            <main className="lg:ml-24 pt-20 min-h-screen transition-all duration-500">
              <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 page-enter">
                <Breadcrumb homeHref="/researcher/dashboard" />
                {children}
              </div>
            </main>
          </div>

...
export default function A2UIStressPage() {
  ...
  const getQuizData = (variantB: boolean) => variantB ? ({
    component: "Quiz",
    props: {
  ...
  const getFlashcardData = (variantB: boolean) => variantB ? ({
    component: "Flashcard",
    props: {
  ...
  const getChartData = (variantB: boolean) => variantB ? ({
    component: "Chart",
    props: {
        type: "line",
  ...
  const getTimelineData = (variantB: boolean) => variantB ? ({
      component: "Timeline",
      props: {
          title: "Project Roadmap",
          events: [
              {"date": "Q1 2024", "title": "Planning", "description": "Define scope and requirements"},
              {"date": "Q2 2024", "title": "Development", "description": "Core features implementation"}
          ]
      }
  }) : ({
      component: "Timeline",
      props: {
          title: "History of AI",
          events: [
              {"date": "1956", "title": "Dartmouth Workshop", "description": "Birth of AI as a field"},
              {"date": "2017", "title": "Transformer Model", "description": "Paper 'Attention Is All You Need' published"}
          ]
      }
  });

  const getComparisonData = (variantB: boolean) => variantB ? ({
  ...
  const getCodeBlockData = (variantB: boolean) => variantB ? ({
      component: "CodeBlock",
      props: {
    ...
    const getMermaidData = (variantB: boolean) => variantB ? ({
      component: "Mermaid",
      props: {
          chart: "graph LR; A[Start] --> B{Decision}; B -->|Yes| C[Do Task]; B -->|No| D[End];",
  ...
  const TEST_CASES = [
    ...
    { id: 'flashcard', title: 'Flashcard Component', getData: getFlashcardData },
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-12 pb-24">
      <div className="sticky top-0 z-50 bg-white dark:bg-black p-4 border-b flex justify-between items-center shadow-md">
          ...
          <Button onClick={toggleVariant} variant={useVariantB ? "destructive" : "default"}>
              {useVariantB ? "Switch to State A (Original)" : "Switch to State B (Updated)"}
          </Button>
      </div>
      
      <p className="text-gray-500">
          This page verifies all A2UI components rendering and their adaptability to dynamic data changes.
          Use the toggle above to simulate a full state update.
      </p>
      ...
      {TEST_CASES.map((test) => {
        ...
        return (
            <div key={test.id} className="border p-6 rounded-xl bg-gray-50 dark:bg-gray-900 shadow-sm">
            <h2 className="text-lg font-semibold mb-4 border-b pb-2">{test.title}</h2>
            ...
            <details className="mt-4 text-xs text-gray-500">
                <summary className="cursor-pointer hover:text-gray-700">View Source Payload</summary>
                <pre className="mt-2 bg-gray-200 dark:bg-gray-800 p-3 rounded overflow-x-auto">{jsonString}</pre>
            </details>
            </div>
        );
      })}
       ...
       <div className="border-t pt-8 mt-12">
            <h2 className="text-xl font-bold mb-6 text-red-500">Edge Case Handling</h2>

...
export default function AITutorTestPage() {
  ...
  const [provider, setProvider] = useState<"lumina" | "gemini" | "local">(
    "lumina",
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const query =
        provider === "local" ? `[LOCAL] ${userMsg.text}` : userMsg.text;
      const response = await processMessage(query);
      setMessages((prev) => [
        ...prev,
        { role: "model", text: response.text, source: response.source },
      ]);
    } catch (error) {
      console.error(error);
      setMessages((prev) => [
        ...prev,
        { role: "model", text: "Error procesing request.", source: "rule" },
      ]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 bg-neutral-800 rounded-xl border border-neutral-700 flex flex-col h-[80vh] overflow-hidden">
              ...
              <div className="flex gap-2 mt-2">
                <button
                  ...
                  className={`px-3 py-1 text-xs rounded-full transition-colors ${
                    provider === "lumina"
                      ? "bg-gradient-to-r from-green-400 to-emerald-600 text-white shadow-lg shadow-green-900/20"
            ...
            {messages.length === 0 && (
              <div className="text-center text-neutral-500 mt-20">
                ...
                <p className="text-sm mt-2">
                  Try: "What is Newton's First Law?"
                </p>
              </div>
            )}
            {messages.map((m, i) => (
          ...
          <form
            ...
            className="p-4 bg-neutral-800 border-t border-neutral-700"
          >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-neutral-900/50 p-4 rounded-lg border border-neutral-700/50">
              ...
              <ol className="list-decimal list-inside space-y-2 text-sm text-neutral-300">
                ...
                <li>
                  Note the{" "}
                  <span className="text-purple-400 font-bold">API</span> source
                  tag.
                </li>
                <li>
                  Ask the <strong>same question</strong> again.
                </li>
                <li>
                  Note the{" "}
                  <span className="text-green-400 font-bold">CACHE</span> source
                  tag (Instant).
                </li>
                <li>
                  Ask "Hi there" to see{" "}
                  <span className="text-yellow-400 font-bold">RULE</span> based
                  response.
                </li>
              </ol>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-center">
                <div className="bg-neutral-800 p-2 rounded">
                  <div className="text-2xl font-bold text-green-400">
                    {messages.filter((m) => m.source === "cache").length}
                  </div>
                  <div className="text-[10px] text-neutral-500">Cache Hits</div>
                </div>
                <div className="bg-neutral-800 p-2 rounded">
                  <div className="text-2xl font-bold text-purple-400">
                    {messages.filter((m) => m.source === "api").length}
                  </div>

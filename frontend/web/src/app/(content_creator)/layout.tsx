...
export default function ContentCreatorLayout({ children }: { children: React.ReactNode }) {
  ...
  ? { name: user.name ?? "Creator", role: "Content Creator", initial: (user.name ?? "C").charAt(0), avatar: user.avatar }
  : { name: "Creator", role: "Content Creator", initial: "C" };

  return (
    <RoleGuard allowedRoles={["content_creator"]}>
      <InstitutionGuard>
          ...
          <div className="relative z-10">
            ...
            <main className="lg:ml-24 pt-20 min-h-screen transition-all duration-500">
              <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 page-enter">
                <Breadcrumb homeHref="/content_creator/dashboard" />
                {children}
              </div>
            </main>
          </div>

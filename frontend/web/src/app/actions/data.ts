...
export async function getStudentDashboard(email: string) {
  try {
    ...
    const currentStreak = Math.max(
      ...enrolledCourses.map((c: any) => c.streak || 0),
      0,
    );
      ...
      enrolledCourses.length > 0
        ? Math.round(
            enrolledCourses.reduce(
              (acc: number, curr: any) => acc + (curr.mastery || 0),
              0,
    ...
    const totalHours = enrolledCourses.reduce(
      (acc: number, curr: any) => acc + (curr.hoursSpent || 0),
      0,
    );
    ...
    const achievements = allAchievements.map((ach) => ({
      ...ach,
      unlocked: badges.some(
        (b: any) => b.id === ach.id || b.name === ach.title,
      ),
    }));
  ...
  try {
    ...
    if (updatedProgress >= 100) {
      ...
      if (!existingCert) {
        ...
        const certificateId = `CERT-${Date.now()}-${Math.floor(
          Math.random() * 1000,
        )}`;
...
export async function completeLesson(
  ...
  try {
    ...
    const progressUpdate = await db
      .collection("progress")
      .updateOne({ userId: userId, courseId: courseId }, {
    ...
    if (!module)
      return { success: true, message: "Lesson completed (Module not found)" };

    // Get user's updated completed lessons
    const progressRecord = await db
      .collection("progress")
      .findOne({ userId: userId, courseId: courseId });
    const completedLessons = progressRecord?.completedLessons || [];

    const allLessonsComplete = module.lessons?.every((l: any) =>
    ...
    if (allLessonsComplete && module.lessons && module.lessons.length > 0) {
      ...
      if (!existingBadge) {
        ...
        await db
          .collection("users")
          .updateOne({ _id: user._id }, { $push: { badges: newBadge } } as any);
...
export async function getStudentProgress(email: string) {
  try {
    ...
    const progressWithCourses = await db
      .collection("progress")
      .aggregate([
        { $match: { userId: user._id.toString() } },
        {
    ...
    const currentStreak = Math.max(
      ...progressWithCourses.map((p: any) => p.streak || 0),
      0,
    );
    const avgAccuracy =
      totalCourses > 0
        ? Math.round(
            progressWithCourses.reduce(
              (acc: number, curr: any) => acc + (curr.mastery || 0),
              0,
            ) / totalCourses,
          )
    ...
    const totalXP = progressWithCourses.reduce(
      (acc: number, curr: any) => acc + (curr.progress || 0) * 10,
      0,
    );
  ...
  try {
    ...
    return serializeMongoObject(
      students.map((student) => {
        ...
        const studentProgress = progressDocs.filter(
...
export async function getStudentCourses(email: string) {
  try {
    ...
    return serializeMongoObject(
      courses.map((course) => {
        const progress = enrollments.find(
...
export async function getExploreCourses(email: string) {
  try {
    ...
    return {
      enrolled: serializeMongoObject(
        enrolled.map((c) => ({ ...c, id: c._id.toString() })),
      ),
      recommended: serializeMongoObject(
        recommended.map((c) => ({ ...c, id: c._id.toString() })),
      ),
    };
  } catch (e) {
...
export async function inviteStudentToCourse(
  ...
  try {
    ...
    return {
      success: true,
      message: `Successfully added ${student.name} to the course.`,
    };
  } catch (e) {
...
export async function addLesson(
  ...
  try {
    ...
    const result = await db
      .collection("courses")
      .updateOne({ _id: new ObjectId(courseId), "modules.id": moduleId }, {
...
export async function getAdminDashboard(email: string) {
  try {
    ...
    console.warn(
      "Warning: Failed to fetch admin dashboard data (likely no DB connection). Serving mock data.",
    );
...
export async function getCourseDetails(courseId: string) {
  try {
    ...
    return serializeMongoObject({
      ...
      instructorId: course.instructorId,
      expandedDescription:
        course.expandedDescription || "No detailed description available.",
...
export async function saveChatMessage(
  email: string,
  message: { sender: string; text: string; sessionId?: string },
) {
  try {
    const client = await clientPromise;
    const db = client.db("lumina_db");
...
export async function deleteModule(
  ...
  try {
    ...
    const result = await db
      .collection("courses")
      .updateOne({ _id: new ObjectId(courseId) }, {
...
export async function getAllStudentsWithProgress() {
  try {
    ...
    const courseMap = new Map(
      allCourses.map((c) => [c._id.toString(), c.name]),
    );

...
export default function PeerTutorLayout({ children }: { children: React.ReactNode }) {
  ...
  ? { name: user.name ?? "Peer Tutor", role: "Peer Tutor", initial: (user.name ?? "P").charAt(0), avatar: user.avatar }
  : { name: "Peer Tutor", role: "Peer Tutor", initial: "P" };

  return (
    <RoleGuard allowedRoles={["peer_tutor"]}>
      <InstitutionGuard>
          ...
          <div className="relative z-10">
            ...
            <main className="lg:ml-24 pt-20 min-h-screen transition-all duration-500">
              <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 page-enter">
                <Breadcrumb homeHref="/peer_tutor/dashboard" />
                {children}
              </div>
            </main>
          </div>

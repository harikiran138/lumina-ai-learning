      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {trustPoints.map((point, index) => (
            <div key={index} className="glass-v2-primary p-8 group transition-all duration-500 hover:border-primary/40">
              ...
              <h3 className="text-xl font-bold text-text mb-4 font-display">{point.title}</h3>
        ...
        <div className="mt-20 p-8 glass-panel flex flex-col md:flex-row items-center justify-between gap-8">
           <div className="flex items-center space-x-6">
              <div className="h-12 w-12 rounded-full bg-surface-elevated border border-border flex items-center justify-center text-text font-black">L</div>
              <p className="text-sm text-text-secondary font-sans italic">"Lumina's privacy architecture sets the gold standard for AI in education." — Dr. Sarah Chen, Research Governance</p>
           </div>
           <div className="flex space-x-8 opacity-40">
              <div className="text-[10px] font-black text-text py-2 px-4 border border-border rounded">SOC2 TYPE II</div>
              <div className="text-[10px] font-black text-text py-2 px-4 border border-border rounded">HIPAA COMPLIANT</div>
           </div>
        </div>
      </div>

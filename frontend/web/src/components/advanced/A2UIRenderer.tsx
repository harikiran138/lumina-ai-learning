...
const QuizSchema = z.object({
  ...
  difficulty: z
    ...
    .optional()
    .default("medium"), // Case insensitive & optional
});
...
const QuizComponent = ({
  ...
  difficulty,
  onAction,
}: z.infer<typeof QuizSchema> & { onAction?: any }) => {
      ...
      >
        <div>
          <div className="flex gap-2 mb-1">
            <Badge
              variant="outline"
              className="text-[10px] h-5 px-1.5 border-white/10 text-gray-400"
            >
              {topic}
            </Badge>
            <Badge
              variant="outline"
              className="text-[10px] h-5 px-1.5 border-white/10 text-gray-400 capitalize"
            >
              {difficulty}
            </Badge>
          </div>
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div className="flex gap-2 items-center">
          <div className="w-8 h-8 rounded-lg bg-lumina-primary/20 flex items-center justify-center">
            <Sparkles className="w-4 h-4 text-lumina-primary animate-pulse" />
          </div>
        ...
        {!isSubmitted && (
          <Badge
            variant="outline"
            className="border-white/5 bg-white/5 text-gray-500 font-mono text-[9px] h-6"
          >
            Active Session
          </Badge>
        )}
      ...
      <h3 className="text-xl font-bold text-white mb-8 leading-relaxed tracking-tight whitespace-pre-line">
        {question}
      </h3>
      ...
      <div className="space-y-3 relative z-10">
        {options.map((opt, i) => {
          let btnClass =
            "w-full text-left p-4 rounded-xl text-sm border transition-all duration-300 relative overflow-hidden group ";

          if (isSubmitted) {
            if (i === correctIndex)
              btnClass +=
                "bg-green-500/10 border-green-500/40 text-green-100 ring-1 ring-green-500/50";
            else if (i === selected)
              btnClass +=
                "bg-red-500/10 border-red-500/40 text-red-100 ring-1 ring-red-500/50";
            else
              btnClass +=
                "bg-white/5 border-white/5 text-gray-500 opacity-40 grayscale";
          } else {
            if (i === selected)
              btnClass +=
                "bg-lumina-primary/10 border-lumina-primary/50 text-white ring-1 ring-lumina-primary/50 shadow-[0_0_15px_rgba(34,197,94,0.1)]";
            else
              btnClass +=
                "bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:border-white/20 hover:pl-5"; // Indent on hover
          }
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4 relative z-10">
                  ...
                  className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold border transition-colors ${
                    isSubmitted && i === correctIndex
                      ...
                      : isSubmitted && i === selected
                        ...
                        : i === selected
                          ? "bg-lumina-primary border-lumina-primary text-black"
                ...
                {isSubmitted && i === correctIndex && (
                  <CheckCircle className="w-5 h-5 text-green-500 absolute right-0" />
                )}
                {isSubmitted && i === selected && i !== correctIndex && (
                  <XCircle className="w-5 h-5 text-red-500 absolute right-0" />
                )}
              </div>
            </button>
          );
        ...
        <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-gradient-to-br from-[#1a1a2e] to-[#16213e]">
          ...
          <div className="h-full flex flex-col p-8 relative z-10">
            <div className="flex justify-between items-center mb-6">
              <span className="text-[10px] font-bold tracking-[0.2em] text-lumina-primary uppercase border border-lumina-primary/30 px-2 py-1 rounded">
                Question
              </span>
        ...
        >
          ...
          <div className="h-full flex flex-col p-8 relative z-10">
            <div className="flex justify-between items-center mb-4">
              <span className="text-[10px] font-bold tracking-[0.2em] text-green-400 uppercase border border-green-500/30 px-2 py-1 rounded">
                Answer
              </span>
            </div>
...
const CourseCardComponent = ({
  ...
  description,
  matchScore,
}: z.infer<typeof CourseCardSchema>) => {
  return (
...
const YoutubeVideoComponent = ({
  ...
  return (
    <div className="my-4 rounded-xl overflow-hidden border border-white/10 bg-black/40">
      {title && (
        <div className="p-2 px-3 text-sm text-gray-300 bg-white/5 border-b border-white/5 flex items-center gap-2">
...
const MermaidComponent = ({ chart, title }: z.infer<typeof MermaidSchema>) => {
  ...
  const sanitizeMermaid = (code: string) => {
    // 1. Fix unquoted text with parentheses in nodes
    // Pattern: NodeID(Text with (parens)) -> NodeID("Text with (parens)")
    // This is a naive regex but handles common cases
    let sanitized = code.replace(/(\w+)\(([^")]*?\([^")]*?\)[^")]*?)\)/g, '$1("$2")');
    
    // 2. Fix unquoted text with [] in nodes
  ...
  React.useEffect(() => {
    const renderChart = async () => {
      try {
        ...
        const { svg } = await mermaid.render(`mermaid-${id}`, safeChart);
        setSvg(svg);
      } catch (e) {
        console.error("Mermaid Render Error", e);
        setSvg(
          '<div class="text-red-500 text-xs p-2">Failed to render chart</div>',
        );
      }
    };
    renderChart();
    ...
    <div className="my-6 rounded-xl border border-white/10 overflow-hidden bg-[#0d1117]">
      ...
      {svg.includes("text-red-500") && (
        <details className="p-2 border-t border-white/10">
          <summary className="text-[10px] text-gray-500 cursor-pointer hover:text-white">
            Debug: Raw Mermaid Code
          </summary>
...
const TimelineComponent = ({
  ...
  return (
    <div className="my-8 relative">
      <h3 className="text-center text-lg font-bold text-white mb-8 border-b border-white/10 pb-2 inline-block mx-auto px-8 relative left-1/2 -translate-x-1/2">
        {title}
      </h3>
        ...
        {(Array.isArray(events) ? events : []).map((ev, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`ml-10 md:ml-0 w-full md:w-5/12 p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all group ${
                  isLeft ? "md:text-right" : "md:text-left"
                }`}
              >
                <span className="text-xs font-mono text-lumina-primary/80 block mb-1">
                  {ev.date}
                </span>
                <h4 className="text-base font-bold text-white group-hover:text-lumina-primary transition-colors">
                  {ev.title}
                </h4>
...
const ComparisonTableComponent = ({
  ...
  return (
    <div className="my-4 rounded-xl border border-white/10 overflow-hidden bg-white/5">
      ...
      <div
        className="grid bg-white/5 text-xs font-bold text-gray-400 uppercase tracking-wider"
        style={{
      ...
      <div className="divide-y divide-white/5">
        {rows.map((row, i) => (
          <div
            key={i}
            className="grid text-sm hover:bg-white/5 transition-colors"
            style={{
              gridTemplateColumns: `repeat(${headers.length}, minmax(0, 1fr))`,
            }}
          >
            <div className="p-3 border-r border-white/5 text-gray-500 text-xs font-mono flex items-center justify-center font-bold uppercase">
              {row.feature}
            </div>
            {(Array.isArray(row.values) ? row.values : []).map((val, j) => (
              <div
                key={j}
                className="p-3 border-r border-white/5 last:border-r-0 text-gray-300 text-center flex items-center justify-center"
              >
                {val}
              </div>
            ))}
            ...
            {rows.map((row, i) => (
              ...
              >
                {row.map((cell, j) => (
                  <td
                    key={j}
                    className="px-6 py-4 text-gray-300 font-medium whitespace-pre-wrap"
                  >
                    {cell}
                  </td>
                ))}
...
const PPTDownloadComponent = ({
  ...
  fileSize,
  slideData,
}: z.infer<typeof PPTDownloadSchema>) => {
    ...
    ? [
        {
          ...
          bullets: [slideData.subtitle || ""] as string[],
          isTitleSlide: true,
        },
                ...
                <div
                  className={`p-4 border-b-4 ${
                    allSlides[currentSlideIndex].isTitleSlide
                      ? "bg-lumina-primary h-full flex flex-col justify-center items-center text-center border-none"
                      : "bg-white border-lumina-primary"
                  }`}
                >
                  <h2
                    className={`${
                  ...
                  {allSlides[currentSlideIndex].isTitleSlide && (
                    <p className="text-xl text-white/90 mt-4">
                      {allSlides[currentSlideIndex].bullets[0]}
                    </p>
                  )}
                </div>
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      setCurrentSlideIndex(
                        Math.min(allSlides.length - 1, currentSlideIndex + 1),
                      );
                    }}
...
const ScoreCardComponent = ({
  ...
  topic,
  message,
}: z.infer<typeof ScoreCardSchema>) => {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
    ...
    >
      ...
      <div className="flex items-center gap-4 mb-6 relative z-10">
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <Badge className="bg-lumina-primary/20 text-lumina-primary border-lumina-primary/20">
              Score: {score}
            </Badge>
          </div>
        </div>
      ...
      <p className="text-gray-300 text-sm text-center italic relative z-10">
        {message ||
          "Great job completing the quiz! Keep learning to improve your score."}
      </p>
    </motion.div>
  );
};
...
export const A2UIRenderer = memo(
  ({
    ...
    const isValidBlock = (item: any) => {
      return (
        ...
        (item.type === "text_block" ||
          item.type === "quiz_block" ||
          item.type === "flashcard_block" ||
          item.type === "diagram_block" ||
          item.type === "table_block" ||
          item.type === "step_block" ||
          item.component)
      );
    };

    // Helper to render a list of blocks
    const renderBlocks = (items: any[], keyPrefix: string) => {
        ...
        return (
          ...
          >
            {items.map((item, index) => {
              ...
              return (
                <motion.div variants={itemVariants} key={`${keyPrefix}-${index}`}>
                  <A2UIErrorBoundary>
    ...
    try {
      if (
        ...
        (trimmed.startsWith("{") && trimmed.endsWith("}"))
         ...
         if (items.some(isValidBlock)) {
            return renderBlocks(items, "pure-json");
         }
      }
    } catch (e) {
    ...
    // Strategy 2: Look for embedded JSON arrays [ ... ]
    // This is common when LLM outputs intro text then the JSON
    try {
      ...
      if (firstBracket !== -1 && lastBracket > firstBracket) {
        ...
        if (potentialJson.includes("{") && potentialJson.includes("}")) {
           ...
           if (Array.isArray(parsed) && parsed.length > 0 && parsed.every(isValidBlock)) {
              // Valid embedded JSON found!
              const preText = content.substring(0, firstBracket).trim();
              const postText = content.substring(lastBracket + 1).trim();
            ...
            try {
              ...
              if (Array.isArray(parsed)) {
                return (
                  <div key={index} className="space-y-4">
                     {/* Reuse logic would be better but keeping it simple for replacement */}
              ...
              if (part.trim().endsWith("```")) {
                return (
                  <div
                    key={index}
                    className="p-3 border border-red-500/20 bg-red-500/5 rounded-xl flex items-center gap-3 text-red-300 text-xs font-mono my-4"
                  >
                    <AlertTriangle className="w-4 h-4 shrink-0" />
                    <div>
                      <p className="font-bold mb-1">UI Generation failed</p>
              ...
              return (
                <div
                  key={index}
                  className="p-3 border border-lumina-primary/30 bg-lumina-primary/10 rounded-xl text-xs text-lumina-primary font-mono my-4 flex items-center gap-2"
                >
...
const renderComponentItem = (data: any, index: number, onAction?: any) => {
  ...
  try {
    switch (data.component) {
      case "Quiz":
        component = (
          <QuizComponent key={uniqueKey} {...props} onAction={onAction} />
        );
        break;
      case "Flashcard":
      ...
      default:
        component = (
          <div
            key={uniqueKey}
            className="p-2 border border-yellow-500/50 bg-yellow-500/10 rounded text-xs text-yellow-200 font-mono"
          >
    ...
    component = (
      <div
        key={uniqueKey}
        className="p-2 border border-red-500/50 bg-red-500/10 rounded text-xs text-red-200 font-mono"
      >

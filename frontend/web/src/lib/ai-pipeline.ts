...
export function chunkPages(
  pages: PageText[],
  maxChars = 20000,
): { text: string; startPage: number; endPage: number }[] {
...
async function callWebLLM(
  ...
  try {
    ...
    if (firstBrace === -1 || lastBrace === -1) {
      console.warn(
        "No JSON braces found in AI response:",
...
export async function extractSectionsFromChunk(
  chunkText: string,
  engine: MLCEngineInterface,
): Promise<any> {
  const prompt = `
...
`;
  ...
  try {
    ...
    if (parsed.success) {
      ...
      console.warn(
        "Schema validation failed, but using partial data:",
        parsed.error,
      );
...
export async function mergeSectionsToCourse(
  allSections: any[],
  engine: MLCEngineInterface,
): Promise<any> {
  // If we have nothing, abort
  if (!allSections || allSections.length === 0)
    throw new Error("No sections to merge.");
    ...
    topics: mod.topics?.map((topic: any) => {
      ...
      if (original) {
        ...
        if (original.summary) {
          ...
          content = [
            { type: "paragraph", data: `Overview of ${original.title}` },
          ];
        }
      }

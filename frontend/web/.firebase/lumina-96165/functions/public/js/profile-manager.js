...
class ProfileManager {
  ...
  async getUserStatistics(userId) {
    const progressData = await window.luminaDB.getByIndex(
      "progress",
      "studentId",
      userId,
    );
    const notes = await window.luminaDB.getByIndex(
      "notes",
      "studentId",
      userId,
    );
    const enrolledCourses = progressData.length;
    const totalHoursLearned = progressData.reduce(
    ...
    const averageMastery =
      progressData.length > 0
        ? Math.round(
            progressData.reduce((sum, p) => sum + (p.mastery || 0), 0) /
              progressData.length,
          )
        : 0;

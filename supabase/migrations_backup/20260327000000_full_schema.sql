...
-- LUMINA V2 FINAL SCHEMA PATCH
-- Applies all production-level structural fixes safely on top of existing database
...
-- Seed basic roles
INSERT INTO roles (name) VALUES ('admin'), ('hod'), ('teacher'), ('student'), ('parent'), ('counselor') ON CONFLICT DO NOTHING;

-- 2. Class Structure Amplification
ALTER TABLE classes ADD COLUMN IF NOT EXISTS semester_id UUID REFERENCES semesters(id) ON DELETE CASCADE;
ALTER TABLE classes ADD COLUMN IF NOT EXISTS department_id UUID REFERENCES departments(id) ON DELETE CASCADE;
ALTER TABLE classes ADD COLUMN IF NOT EXISTS batch_year TEXT; -- e.g. '2023-2027'

-- 3. Student Subject Explicit Mapping
...
-- 4. Course Ownership & Scoping
ALTER TABLE courses ADD COLUMN IF NOT EXISTS department_id UUID REFERENCES departments(id) ON DELETE CASCADE;
ALTER TABLE courses ADD COLUMN IF NOT EXISTS teacher_id UUID REFERENCES public.users(id) ON DELETE SET NULL;

-- 5. Knowledge Graph Linkages (Concept Edges)
CREATE TABLE IF NOT EXISTS concept_edges (
    from_concept UUID REFERENCES course_concepts(id) ON DELETE CASCADE,
    to_concept UUID REFERENCES course_concepts(id) ON DELETE CASCADE,
    relation_type TEXT, -- 'prerequisite', 'related'
    PRIMARY KEY(from_concept, to_concept, relation_type)
);

-- 6. AI Queue Priority System
ALTER TABLE ai_answer_queue ADD COLUMN IF NOT EXISTS priority_score INT DEFAULT 0;
...
-- 8. Program Durations & Tracking Engine
ALTER TABLE programs ADD COLUMN IF NOT EXISTS duration_years INT DEFAULT 4;
...
-- 9. Clean State Machine for Approvals (Teacher Requests)
ALTER TABLE teacher_requests ADD COLUMN IF NOT EXISTS verified_by UUID REFERENCES public.users(id) ON DELETE SET NULL;
ALTER TABLE teacher_requests ADD COLUMN IF NOT EXISTS verified_at TIMESTAMPTZ;

-- 10. Explicit Indexes for Scaling
